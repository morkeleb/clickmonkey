...
| **Silent Save** | Submit/Save: no navigation, no write request, and no accessible invalid (`aria-invalid`, `{id}-error`, HTML5 constraint). WCAG 3.3.1. Not Existing/Create-new mode toggles (`*_mode_*`) | Unleash after submit |
| **Invalid accepted** | Required blank, or typed junk a control should not accept (email/date/number/`pattern`): after Save the field is not invalid, **and** the form sent that value or left the page. XSS/SQLi in a free-text notes field is not this | Unleash / `empty-required`. Catalog **Q-02**. |
...
|---|---|---|
| **overflow** | Page or container leaking past its edge (document wider than the viewport past the scrollbar gutter; ~40px+ is high). Also at **375px** (phone) and **320px** (WCAG 1.4.10 reflow). Findings tagged `@ 320px` / `@ 375px` attach a still **at that width**, not the 1280 step shot | Intended `overflow: auto` / `scroll`, closed off-canvas drawers, sticky `100vh` chrome, `overflow: hidden` clip (that is **clip**), data tables that need two-axis scroll |
| **clip** | Text cut mid-word with no `…` — table cells (including overflow on an inner span, not only the `td`), **column headers that collide or overflow into the next header**, fields, tabs, chips, headings. Also a value or tab title whose glyphs collide with a trailing icon / `$` / `%` inside a box that still “fits”. Finding still is a crop around the control | Clean ellipsis, scrollable panes, roomy inputs whose letters are readable; native date/color UA picker icons; a table cell whose only overflow is the editor input (that is field clip); a header that wraps onto two lines inside its own column; **collapsed icon-rail labels** (icon-sized item ≤72px with an icon in nav/aside, `data-sidebar`, or a flush viewport-edge strip — the word is not meant to show) |
| **overlap** | Two actable controls sharing ≥8×8 px | Parent/child, labels on labels, an open menu covering the page, **header/nav chrome crowding** (Sign Out vs sidebar — quality ledger, not a finding) |
...
| **targetSize** | Actable control smaller than 24×24 CSS px on **both** axes, unless a 24px circle around it misses every other target (WCAG 2.5.8 spacing). Finding still is a crop around the control | Inline text links, native checkbox/radio/file, isolated icons with enough gap, disabled, a wrapping label ≥24×24, **1×1 / sr-only native `<select>`** (and a 1×1 Close) under a custom combobox — the painted trigger is the target, **Close × nested in a ≥24px tab chip** (`Close {title}` / 18×18 tab-strip × stays on the quality ledger, not a finding) |
...
| **textOcclusion** | Heading/body text fully covered by a painted sibling (badge, overlay). Finding still is a crop around the cell | Partial corner overlap; clip inside the same box; actable overlap; **chips restacked over the same cell** (same labels, or an empty Tailwind `min-w-0` / `font-medium` / `after:absolute` wrapper) |
...
Fence bounce (`/logout`, off-app URL) is leash control, not a product finding. After a self-logout the walker `goto`s the leash `url` and runs `intro` again.
...
Unleash after submit also flags **typed junk** the control should not accept: HTML `type` (email/url/number/date), `pattern`, `min`/`max`/`minlength`/`maxlength`. If the form sent those values or left without marking the fields invalid, that is the same class as empty-required. `--nasty` XSS/SQLi in a text or textarea field is **not** Q-02 when the product stores it as text (no crash, no injection). A crash on that fill is Q-03. The `empty-required` playbook still `expect`s visible invalid marks even when the form stays put.
